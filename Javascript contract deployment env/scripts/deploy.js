...
async function main() {
    ...
    const cal = await CalFundToken.deploy(deployer.address);
    await cal.deployed();
    console.log('CalFundToken deployed at:', cal.address);

    // Create a pair
    const tx1 = await factory.createPair(cal.address, weth.address);
    await tx1.wait();
    const pairAddress = await factory.getPair(cal.address, weth.address);
