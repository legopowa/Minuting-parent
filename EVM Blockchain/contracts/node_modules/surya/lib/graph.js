...
function graph(files) {
  ...
  var _loop2 = function _loop2(ast) {
    ...
    function nodeName(functionName, contractName) {
      if (functionName !== '<Fallback>' && functionName !== '<Receive Ether>' && functionName !== '<Constructor>' && dependencies.hasOwnProperty(contractName)) {
      ...
      FunctionCall: function FunctionCall(node) {
        ...
        if (parserHelpers.isRegularFunctionCall(node, contractNames, eventsOfDependencies, structsOfDependencies)) {
          ...
          if (object === null) {
            ...
            var _matchingContracts2 = [].concat(_toConsumableArray(dependencies[contractName])).filter(function (contract) {
              return functionsPerContract[contract].includes(name);
            });

            if (_matchingContracts2.length > 0) {
              localContractName = _matchingContracts2[0];
            } else {
              return;
            }
