...
class LamportTest:
    ...
    def load_two_oracles(self, pkhs, filename):
        ...
        while not oracle1_loaded and pkh_index < len(pkhs):
            try:
                ...
                print(f"oracle pkh index =", pkh_index)
                pkh_index += 1  # increment the pkh_index after successful load
            except InvalidAddress:
    ...
    def can_test_key_functions(self, accs):
        ...
        ## The start of the repeating segment (repeats for every entry in tfc_servers.txt)
        with open('tfc_servers.txt', 'r') as f:
            server_ips = [line.strip() for line in f.readlines()] 
        
        for server_IP in server_ips:      

            current_keys = self.k4.load(self, "Activityoracle1", oracle_pkh_2)
...
def handle_event(event):
    print(f"New OracleKeyIndicesRetrieved event: {event}")

def main():
    # Initialize and run the LamportTest
    lamport_test = LamportTest()
    lamport_test.can_test_key_functions([str(acc) for acc in accounts])

    # Create an event filter for OracleKeyIndicesRetrieved
    event_filter = lamport_test.contract.events.OracleKeyIndicesRetrieved.createFilter(fromBlock='latest')
