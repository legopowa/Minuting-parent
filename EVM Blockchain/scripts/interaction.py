...
def main():
    ...
    cal = CalFundToken.at(addresses['cal'])
    ...
    cal.approve(router.address, amount_in_cal, {'from': deployer}).wait(1)
    ...
    amount_in_weth = web3.to_wei(0.2, 'ether')  # 0.2 WETH
    amount_out_min_cal = 0  # Accept any amount of CAL

    weth.approve(router.address, amount_in_weth, {'from': deployer}).wait(1)
